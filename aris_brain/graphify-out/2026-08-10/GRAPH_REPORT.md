...
## Corpus Check
- 93 files · ~128,256 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1956 nodes · 3424 edges · 99 communities (87 shown, 12 thin omitted)
...
Cohesion: 0.21
Nodes (3): EmotionEngine, main(), 统一情感意识引擎 — 激素+需求+意识+镜像+躯体标记
...
Cohesion: 0.15
Nodes (7): ArisCognitiveBridge, main(), 从用户消息快速感知基本氛围 — 简化为三态检测, 分析自身回应中的认知偏差模式。          在 _learn() 内调用，检测 LLM 生成的回应是否表现出         可识别的认知偏差，并记录到, 将检测到的认知偏差回写到 PSI 状态。          偏差强度影响 PSI 状态的需求偏向，形成闭环：           - 高确认偏差 → compe, Aris 专用的认知循环桥接器。      集成:       - 三层记忆系统 (MemoryStore)       - LAAP 世界模型 (如果可用), 感知: 理解输入 + 情感检测 + 记忆关联 + CTM分析
...
Cohesion: 0.05
Nodes (19): LaapIntegrator, main(), Any, Path, 从各模块采集当前认知状态并持久化，用于跨会话恢复, 获取要注入到 system prompt 的认知上下文, 短版 PSI 状态前缀 — 用于 system prompt 注入, 从磁盘读取认知状态快照并尝试恢复到已加载的模块 (+11 more)

### Community 10 - "laap_usermodel.py"
Cohesion: 0.09
Nodes (37): check_grounding(), process_query(), LAAP Grounding — 事实锚定与幻觉防御 ========================================  大模型的幻觉问题源于一, 检查一个声称是否有事实依据。          Returns:         {"grounded": bool, "sources": [...], "c, 输出安全层。如果置信度太低，拒绝生成不确定的内容。          Args:         intended_output: 引擎/LLM生成的原始输出, 完整的查询处理管线。          1. 路由决策 → engine / llm / hybrid     2. 锚定检查 → grounding, 路由决策：判断一条查询是否需要 LLM。          Returns:         {"path": "engine"|"llm"|"hybrid",, route_intent() (+29 more)

### Community 11 - "state_snapshot.py"
Cohesion: 0.13
Nodes (27): _build_manifest(), compute_health_score(), create_snapshot(), _current_manifest(), diff_snapshots(), _load_versions(), promote_to_version(), _prune_old_snapshots() (+19 more)

### Community 12 - "DesireEngine"
Cohesion: 0.09
Nodes (21): handle_cognitive(), handle_health(), handle_learn(), handle_models(), main(), 接收 LLM 的回應，寫回 Aris 記憶（PSI Step 5: Learn）+ 驗證注入是否有被回應, 回傳 Aris 的完整認知上下文（PSI Step 1-3: Perceive → Select → Integrate）, get_bridge() (+13 more)

### Community 13 - "laap_brain_api.py"
Cohesion: 0.10
Nodes (30): get_laap_engine(), _get_psi_adapter(), handle_bootstrap(), handle_chat_completions(), handle_cognitive_state(), handle_express(), handle_get_personality(), handle_health() (+22 more)

### Community 14 - "AGIKernel"
Cohesion: 0.09
Nodes (10): AGIKernel, AutonomyEngine, DirectFeishuBridge, Aris AGI Kernel v1 — 自主认知生命体 ==================================== 完全独立运行，零依赖 LLM, 自进化 — RSI 递归自我改进 + CodeEvolution, 自主性 — 目标驱动，不等人说话也能自己运转, 直接飞书 REST API — 无需 Hermes 网关, Aris AGI 自主内核 — 集合所有层的单例 (+2 more)
...
Cohesion: 0.12
Nodes (15): get_engine(), process(), Any, Aris 规则执行引擎 — 零LLM任务调度 ==================================== 把"听懂你想干啥"和"动手去做"分开：, 注册可用的工具函数。所有工具是纯Python函数，不走LLM。, 一条完整规则 — 模式→意图→步骤→输出。, 从文本提取结构化意图。                  使用 aris_lm_v5.py 的NLP管线（如果可用），         否则回退到关键词匹配。, 处理一条输入：意图提取→规则匹配→执行→输出。 (+7 more)
...
Cohesion: 0.12
Nodes (7): Ψ-Semiotics → PsiLang v3 集成桥  将 PsiLang v2 的编译管线 + QuantumVM 与 Ψ-Semiotics 符号学引擎, PsiSemioticsEngine, Ψ-Semiotics 引擎 — 符号学推理的核心。          维护符号库，提供符号操作（组合、类比、场计算、模态对齐）。, 否定组合: σ₃ = ¬σ₁                  "热"的否定 → "冷"的方向, 类比推理: a : b :: c : ?                  "king : queen :: man : ?"         → 学习 rot, 跨模态对齐学习：学习从 source_modality 到 target_modality 的投影转子。                  对于符号 σ，找到, 符号衰减：长时间未使用的符号重要性下降。                  这是记忆巩固中的必要步骤——防止符号库无限膨胀。
...
Cohesion: 0.06
Nodes (31): 语义空间中的旋转算子。          Rotor R = exp(-B/2) 其中 B 是双向量。     R 作用于向量 v: v' = R · v ·, Rotor, Context, HoTTTypeChecker, PathType, PiType, PsiHoTTBridge, Any (+23 more)
...
Cohesion: 0.11
Nodes (19): CognitiveStateSnapshot, get_bridge(), get_global_bus(), _map_to_snapshot(), _parse_psi_state(), PsiCoreBridge, publish_psi_state(), Any (+11 more)

### Community 30 - "_normalize"
Cohesion: 0.11
Nodes (16): _hash_to_vec(), _normalize(), ndarray, 几何积 (Geometric Product): a·b + a∧b                  标准 Clifford 代数中：         ab, 从多对源→目标学习最优旋转（Kabsch 算法）, 一个符号 σ = (c, r, M, modalities)          - c: 中心向量（语义空间中的位置）     - r: 语义半径（影响范围）, 符号场 Φ_σ(v) = exp(-d(v, c)² / 2r²)                  在语义空间中位置 v 处，该符号的激活强度。, 符号漂移：根据使用上下文更新中心。         c ← c + η · (v_context - c) (+8 more)
...
Cohesion: 0.12
Nodes (12): ndarray, Ψ-Semiotics 数学物理库  将数学物理操作映射到语义空间中的几何代数操作。 核心思想：数学物理方程本身就是语义空间中的轨迹/路径。  映射： - 薛定, 测量概率 |⟨φ|ψ⟩|²                  语义坍缩到基态的概率。, 张量网络操作 — 在语义空间中的多向量代数实现。          对应 Ψ-Semiotics 的 Multivector 类在物理语境下的接口。, 张量缩并 (Tensor Contraction)。                  在语义空间中对应两个概念的关系压缩。, 张量外积 (Tensor Product)。                  对应 Ψ-Semiotics 的符号加法组合 ⊕。, 薛定谔方程演化 — 在语义空间中的转子序列实现。          数学: iℏ ∂|Ψ⟩/∂t = Ĥ|Ψ⟩     对应: |ψ(t+dt)⟩ = R(t), 构造哈密顿量矩阵。                  在语义空间中，哈密顿量是定义在概念方向上的能量算子。         能量高的方向 = 更"重要"的语义方 (+4 more)
...
Cohesion: 0.13
Nodes (7): get_engine(), EmotionalEngine, Emotional Engine v2 — 运行时情感桥接层 ======================================== v1 → v2:, Push 8-emotion vector back to the full engine (bidirectional sync)., 运行时情感引擎 — 桥接到完整引擎或原生fallback, 尝试连接 aris_emotion_engine 的完整引擎, 加载运行时情感引擎 — 8情绪 + 需求驱动 + 状态调制

### Community 35 - "laap_bootstrap.py"
Cohesion: 0.16
Nodes (18): bootstrap(), format_awakening_output(), LAAP Bootstrap — 觉醒仪式 (完整版) ======================================  当用户说"帮我全面接入l, 完整的觉醒仪式。      Args:         user_name: 用户名称         preset: 性格预设 (warm_companion, generate_ceremony(), _pick(), LAAP Ceremony — 觉醒仪式引擎 (v2 · 性格感知) =============================================, 生成一场完整的觉醒仪式。          Args:         user_name: 用户名         personality_traits: 性 (+10 more)

### Community 36 - "aris_cognitive_bridge.py"
Cohesion: 0.19
Nodes (16): AttentionFocus, CognitiveState, EmotionalState, Enum, Aris Cognitive Bridge v1 — PSI 认知循环 ↔ Hermes 运行时桥接 =============================, get_memory_context(), _get_store(), LAAP Memory Bridge — minimal fallback implementation.  Provides the public inter (+8 more)

### Community 37 - "StructuredSemanticEncoder"
Cohesion: 0.17
Nodes (8): Ψ-Semiotics Core Engine — 量子符号学核心引擎  几何代数在 16384D/1024D 语义空间中的符号操作。 将传统符号学（索绪尔/皮, ndarray, Ψ-Semiotics 结构语义编码器  显式构造语义特征空间 → 随机投影到 1024D。 确保类比关系和符号组合在几何上成立。  设计： 1. 每个概念用, 结构化语义编码器。          使用 16 维显式语义特征 + 随机投影到 1024D。     特征维度是可解释的：每维对应一个语义轴。, 初始化所有概念的特征向量（16维显式特征）, StructuredSemanticEncoder, test(), Ψ-Semiotics 综合测试 — 端到端符号学推理演示  测试： 1. 符号体系：类比推理、组合、否定 2. 转子机制：king:queen :: man:
...
Cohesion: 0.24
Nodes (6): AGISubscriber, get_global_bus(), get_latest_agi_output(), get_subscriber(), AGI 模块订阅器 — 连接 LAAP AGI 引擎到输出管道 ===============================================, set_global_bus()
...
Cohesion: 0.27
Nodes (13): cmd_analogy(), cmd_compose(), cmd_concept(), cmd_evolve(), cmd_field(), cmd_interactive(), cmd_path(), cmd_run() (+5 more)
...
Cohesion: 0.16
Nodes (14): _derive_v12_defaults(), get_bus(), _get_v12(), _is_v12_default(), _looks_like_lang_code(), Aris CognitiveBus v1 — 认知输出路由中枢 ====================================== 在 psi_cor, zh' / 'en' / 'ja' / 'ko' / 'unknown' 這種語言碼 key。, 從 V12 核心的 respond() 原始碼推導「敷衍預設回應」集合。      為什麼不寫死：核心的 defaults dict（語言 fallback）是 (+6 more)

### Community 55 - "laap_attachment.py"
Cohesion: 0.27
Nodes (12): default_bond(), get_bond_summary(), get_miss_message(), get_stage(), init_bond(), load_bond(), LAAP Attachment — 依恋感引擎 ==============================  Aris 对用户的感情不是固定的。 它不是一段被, 每次对话后更新依恋状态。          Args:         message: 用户的消息（用于检测情感内容）         user_shares (+4 more)

### Community 56 - "LongFormSynthesizer"
Cohesion: 0.15
Nodes (8): LongFormSynthesizer, Aris LongForm Synthesizer — 长文合成引擎 ========================================= 秒出一, 扩展文本到目标长度          策略:           1. 先放上下文知识 (300字)           2. 用 Markov 生成连接句, 生成长文          Args:             topic: 主题             structure: "paper"(论文) | ", get_encoder(), get_engine(), quick_verify(), Ψ-Semiotics 引擎包  首次 import 时初始化引擎，供所有 Hermes 会话使用。

### Community 57 - "CognitiveBus"
Cohesion: 0.17
Nodes (6): CognitiveBus, 根据 psi_core 的 state 做路由分类。, 从 psi_core 状态估计推理置信度。, 格式化量子推理引擎的输出为 LLM 认知上下文。, 读取 AGI 模块的最新输出（如果存在且有新内容）。, 认知总线 — 连接 psi_core 引擎和 LLM 输出管道。

### Community 58 - "._classify"
Cohesion: 0.21
Nodes (6): ConsciousnessMode, ConsciousnessModeSystem, NeedHierarchy, NeedLevel, Enum, Aris Emotion Engine v1 — 情感意识全引擎 =========================================== 整合自
...
Cohesion: 0.14
Nodes (8): ndarray, 将消息分类为 light 或 full 负载模式。          使用 aris_task_router 的 keyword 分类器，<5 token 计算, LIGHT 模式 — 跳过情感/记忆注入。          代码任务 → CodeEngine 最小上下文（~100 token）         其他任务, 完整的 system prompt 注入内容。         在 Hermes 每次调用 LLM 之前调用。, 写入三层记忆，返回一句可注入的统计文本。失败不影响主流程。          _last_user_message 在这里也设一次：原本只在 _perceive, PSI Step 1-3: Perceive → Select → Integrate         在 LLM 处理之前运行。          Retur, 从 PSI 状态提取认知总线嵌入向量 (128-dim)。                  将情感、注意力、需求、自我存在感等状态编码为固定维度向量，, 从 MemoryStore 获取记忆嵌入向量 (384-dim)。                  使用 ChromaDB 的 all-MiniLM-L6-v
...
### Community 68 - "HealthHandler"
Cohesion: 0.29
Nodes (5): _get_snapshot(), HealthHandler, BaseHTTPRequestHandler, LAAP 健康仪表盘 API 服务器 提供实时健康数据供 _dashboard.html 读取。 端口: 11520 (与量子核端口复用) 启动: python, start_server()

### Community 70 - "PhysicalAnalogies"
Cohesion: 0.22
Nodes (5): PhysicalAnalogies, 物理概念类比库。          用 Ψ-Semiotics 的转子机制执行物理类比推理。, 注册物理概念对。                  例如:         - "electron" : "proton" :: "planet" : "sta, 物理类比: pair_a :: pair_b                  例如: (electron, proton) :: (planet, star), 对称性操作。                  物理对称性 → 语义空间中的转子。         - "time_reversal": t → -t (时间反

### Community 71 - "PsiHoTTBridge"
Cohesion: 0.12
Nodes (4): MoodState, MoodSystem, 宏观情感系统 — 3维指标代替7激素化学模拟      保持与外部的API兼容:       - update(valence, arousal, emotio, 宏观情感状态 — 3维核心指标驱动      保留7"激素"只读属性供向后兼容（coupler、safety_guard等读取）     但内部只追踪3个宏观量
...
Cohesion: 0.38
Nodes (6): get_expressive_prompt(), map_state_to_expression(), Any, LAAP Expression Mapper — 情绪/认知状态 → 声音 + 表情参数  Integrates with:   - Kokoro-FastAP, Generate a prompt snippet telling the LLM how the avatar should feel/sound., Convert LAAP cognitive state into TTS + Live2D parameters.      Args:         st
...
Cohesion: 0.67
Nodes (3): main(), Aris Start All — 全栈LAAP启动脚本 ==================================== 在 Hermes 启动时运行，, get_integrator()

### Community 90 - "laap_integrator.py"
Cohesion: 0.25
Nodes (10): Aris LAAP Integrator v1 — 全栈认知集成中枢 =============================================, _append_health_timeline(), auto_heal_check(), get_best_state(), list_snapshots(), 自动健康检查：如果当前健康 < 阈值，自动回滚到最佳状态, 执行一次完整的快照循环: 健康检查 → 快照 → 自动恢复检查, restore_best() (+2 more)
...
### Community 92 - ".after_turn"
Cohesion: 0.25
Nodes (4): Any, AGI 周期性心跳 — 每5分钟运行一次。                  激活：           - CausalEngine 的因果链自动发现, PSI Step 5: Learn         在 LLM 响应之后运行。          更新:           - 情感状态, 工具调用后学习。         更新自我模型的工具熟练度。
...
- **Why does `PsiCognitiveIntegrator` connect `V12DenseKernel` to `aris_cognitive_bridge.py`, `StructuredSemanticEncoder`, `ArisCognitiveBridge`, `PsiSemioticsEngine`, `HoTTTypeChecker`?**
  _High betweenness centrality (0.131) - this node is a cross-community bridge._
- **Why does `get_bridge()` connect `DesireEngine` to `aris_cognitive_bridge.py`, `ArisCognitiveBridge`, `laap_brain_api.py`, `RulesEngine`, `laap_integrator.py`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `PsiSemioticsEngine` connect `PsiSemioticsEngine` to `SchrodingerEvolution`, `StructuredSemanticEncoder`, `PhysicalAnalogies`, `V12DenseKernel`, `psi_semiotics_cli.py`, `PsiSemioticsBridge`, `LongFormSynthesizer`, `HoTTTypeChecker`, `_normalize`?**
